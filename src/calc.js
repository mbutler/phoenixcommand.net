...
export function setUser(user) {
    ...
    _.forEach(weapons, (gun) => {
        $("#weapon-button .col .dropdown-menu").append(
            `<span class="dropdown-item dropdown-eal">${gun.Name}</span>`
        )
    })
    ...
    snap.then((game) => {
        ...
        _.forEach(userCharacters, (player) => {
            if (user.uid === player.userId) {
                $("#calc-character-name .dropdown-menu").append(
                    `<span class="dropdown-item dropdown-eal">${player.characterName}</span>`
                )
            }
        })
    })
...
export function eal() {
    if (
        $("#range").val() === "" ||
        $("#aim-time").val() === "" ||
        $(`#weapon-button .dropdown-toggle`).html() === "Weapon"
    ) {
        Utils.modal(
            "Phoenix Command",
            "Weapon, Range, and Aim Time are required values."
        )
    } else {
        ...
        snap.then((character) => {
            ...
            let firingStance = $(
                "#firing-stance-button .dropdown-toggle"
            ).html()
            let position = $("#position-button .dropdown-toggle").html()
            let situational = Utils.selectedCheckboxes(
                $('[name="situational"]')
            )
            ...
            ;(eal.sal = sal),
                (eal.shotType = shotType),
                (eal.targetSpeed = targetSpeed),
                (eal.shooterSpeed = shooterSpeed),
                (eal.range = range),
                (eal.aimTime = aimTime)
            ;(eal.firingStance = firingStance),
                (eal.position = position),
                (eal.situational = situational),
                (eal.visibility = visibility),
                (eal.targetSize = targetSize)
            ;(eal.weaponAimMod = weaponAim[_.toString(weaponAimIndex)]),
                (eal.targetDiameter = targetDiameter),
                (eal.sab = 0),
                (eal.ammoType = ammoType),
                (eal.salm = 0)
...
function burstHandler(weapon, eal, accuracy, chance) {
    ...
    $(".arc-rows h5")
        .empty()
        .append(
            `Number of Targets in <strong>Minimum Arc of ${getMinimumArc(
                weapon,
                eal.range
            )}:</strong>`
        )
    ...
    $("#odds-label")
        .empty()
        .append(`<strong><h3>Burst Elevation Chance</h3></strong>`)
    ...
    $("#fire-button").click((e) => {
        ...
        if (targets > 0 && arc >= 0) {
            ...
            Utils.modal(
                "Phoenix Command",
                "Arc and number of targets required."
            )
        }
        ...
        $("#sab-message")
            .empty()
            .append("<strong>Sustained Automatic Burst</strong>")
...
function singleShotHandler(weapon, eal, accuracy, chance) {
    ...
    $("#odds-label")
        .empty()
        .append(`<strong><h3>Chance of Hitting</h3></strong>`)
    $("#odds-of-hitting").empty().append(`<h3>${chance}%</h3>`)
    $("#fire-button").click((e) => {
        e.preventDefault()
...
function shotgunHandler(weapon, eal, accuracy, chance) {
    ...
    $("#odds-label")
        .empty()
        .append(`<strong><h3>Pattern Hit Chance</h3></strong>`)
    $("#odds-of-hitting").empty().append(`<h3>${chance}%</h3>`)
    $("#fire-button").click((e) => {
        e.preventDefault()
...
function fireBurst(weapon, numberOfTargets, arc, chance, range) {
    ...
    snap.then((character) => {
        ...
        if (loadedAmmo >= rof) {
            ...
            Utils.log(
                `${character.name} tried to fire a ${weapon.Name} but was out of ammo. ${note}`
            )
            Utils.modal("Phoenix Command", result)
        }
    })
    ...
    snap.then((character) => {
        ...
        if (loadedAmmo >= 1) {
            ...
            if (roll <= chance) {
                ...
                Utils.log(
                    `${character.name} fired a ${weapon.Name} at a target ${range} hexes away with a ${chance}% chance of hitting. ${location} ${note}`
                )
                ...
                Utils.log(
                    `${character.name} fired a ${weapon.Name} ${range} hexes away with a ${chance}% chance. ${location} ${note}`
                )
...
function displayTargets(targetList, weapon, ammoType) {
    ...
    for (let i = 1; i <= _.size(targets); i++) {
        ...
        let location = ""
        bullets =
            _.toNumber($(`#target-${i}-bullets`).text()) +
            _.toNumber(`${targets[`target ${i}`]["bullets"]}`)
        if (bullets > 0) {
    ...
    $("#hits").append(
        '<button id="damage-button" class="btn btn-primary btn-sm">Calculate Damage</button>'
    )
...
function displayExplosionTargets(targetList, weapon, ammoType, location) {
    ...
    $(".target-blast-mod-select").change((e) => {
        ...
        let cover = false
        if (
            blastMod === "Behind Solid Cover" ||
            blastMod === "Under Partial Cover"
        ) {
            cover = true
        }
    ...
    $("#damage-button").click((e) => {
        ...
        _.forEach(radius, (val) => {
            ...
            let blastMod = $(`#target-${val}-blast-mod`)
                .find(":selected")
                .text()
            if (
                blastMod === "Behind Solid Cover" ||
...
function displayDamage(targets) {
    ...
    for (let i = 1; i <= _.size(targets); i++) {
        ...
        if (damage > 0) {
            msg += `Hit target ${i} ${location} for ${damage} damage. `
        }
        if (damage <= 0) {
        ...
        let tr = `
            <tr>
                ...
                <td id="target-${i}-recovery" class="text-center">${pf.medicalAid(
            _.toNumber(damage),
            "No Aid"
        )}</td>
            </tr>
...
function displayExplosionDamage(targets) {
    ...
    _.forEach(radius, (val) => {
        ...
        if (damage > 0) {
            msg += `Hit ${location} from ${val} hexes away for ${damage} damage. `
        }
        location = _.uniq(location)
        let tr = `
            <tr>
                ...
                <td id="target-${val}-recovery" class="text-center">${pf.medicalAid(
            _.toNumber(damage),
            "No Aid"
        )}</td>
            </tr>
