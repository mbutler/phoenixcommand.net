        ...
        gameRef.once("value", (data) => {
            ...
            _.forEach(User.getUserCharacters(game), (player) => {
                ...
                $("#user-table tbody").append(
                    `<tr><td>${link}</td><td>${player.userName}</td><td>${trash}</td></tr>`
                )
            ...
            $("#button-area").append(
                '<button id="download-game-json" class="btn btn-info btn-sm">Download Game</button>'
            )
            $("#download-game-json").click((e) => {
                let title = _.snakeCase(game.metadata.title) + ".json"
                let output = JSON.stringify(game, null, 4)
                let json = [output]
                let blob1 = new Blob(json, { type: "text/plain;charset=utf-8" })
                //Check the Browser.
                let isIE = false || !!document.documentMode
                if (isIE) {
                    window.navigator.msSaveBlob(blob1, title)
                } else {
            ...
            adminRef.once("value").then((snapshot) => {
                snapshot.forEach((childSnapshot) => {
                    ...
                    if (game.metadata.gameId === admin.gameId) {
                        $("#button-area").append(
                            '<button id="delete-game" class="btn btn-danger btn-sm">Delete Game</button>'
                        )
                        $("#delete-game").click((e) => {
                            e.preventDefault()
                            let split = _.split(gameId, "/")
                            let path = `users/${user.uid}/games/`
                            Utils.deleteGameModal(
                                "Phoenix Command",
                                "Delete Game?",
                                path,
                                split[2]
            ...
            $(".timestamp")
                ...
                .append(
                    "created: " +
                        moment
                            .unix(game.metadata.created / 1000)
...
export function addNextImpulseButton(user, game) {
    ...
    adminRef.once("value").then((snapshot) => {
        snapshot.forEach((childSnapshot) => {
            ...
            if (game.metadata.gameId === admin.gameId) {
                $("#time").append(
                    '<button id="next-impulse" class="btn btn-info btn-link">Next Impulse -></button>'
                )
...
export function navList(user) {
    ...
    adminRef.once("value").then((snapshot) => {
        $("#game-dropdown").append(
            '<div class="nav-section"><strong>Created:</strong></div>'
        )
        snapshot.forEach((childSnapshot) => {
            let game = childSnapshot.val()
            $("#game-dropdown").append(
                `<a class="dropdown-item" data-uid="${user.uid}" data-gameid="${game.gameId}" href="#">${game.name}</a>`
            )
        })
    })
    ...
    memberRef.once("value").then((snapshot) => {
        if (snapshot.val()) {
            ...
            $("#game-dropdown").append(
                '<div class="nav-section"><strong>Joined:</strong></div>'
            )
            snapshot.forEach((childSnapshot) => {
                let game = childSnapshot.val()
...
export function addCharacter(character) {
    ...
    snap.then((game) => {
        Database.push(
            "users/" + game.metadata.gameId + "/content/characters/",
            character
        )
...
export function createNew(user, gameName, players) {
    ...
    Database.set(
        "users/" + user.uid + "/games/" + gameId + "/metadata/gameId/",
        user.uid + "/games/" + gameId
    )
    Database.set(
        "users/" + user.uid + "/games/" + gameId + "/users/" + user.uid,
        user.displayName
    )
