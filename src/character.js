...
export function displayCharacterSheet(characterName) {
    ...
    snap.then((game) => {
        ...
        window.localStorage.setItem(
            "phoenix-command-current-character",
            "users/" +
        ...
        $("#character-name")
            .empty()
            .append(`<h3><strong>${character.characterName}</strong></h3>`)
        $("#skill-level")
            .empty()
            .append(`<h6>Level: ${character.skillLevel}</h6>`)
        ...
        $("#cover")
            .val(character.cover)
            .find(`option[value="${character.cover}"]`)
            .attr("selected", true)
        $("#position")
            .val(character.position)
            .find(`option[value="${character.position}"]`)
            .attr("selected", true)
        ...
        $("#stance")
            .val(character.stance)
            .find(`option[value="${character.stance}"]`)
            .attr("selected", true)
        ...
        $("#physical-damage").change((e) => {
            ...
            if (incapEffect.effect !== "") {
                let timeRoll = _.clamp(
                    _.random(0, 9) + incapEffect.timeRollMod,
                    0,
                    9
                )
                ...
                Utils.modal(
                    "Phoenix Command",
                    `${characterName} ${incapEffect.effect} for ${time}`
                )
            }
        })
    })
...
export function displayCharacterCreation() {
    ...
    snap.then((game) => {
        $(".game-title")
            .empty()
            .append(`<a href='game.html'>${game.metadata.title}</a>`)
    })
    ...
    _.forEach(weaponKeys, (gun) => {
        ...
        <div class="form-check form-check-inline">
            <label class="form-check-label">
                <input class="form-check-input" name="weapon" type="checkbox" id="${_.kebabCase(
                    gun
                )}" value="${gun}">${gun}
...
export function displayGear(character) {
    ...
    $("#gear-armor").val(`${character.armor}`)
    $("#change-gear").click((e) => {
        ...
        let path = window.localStorage.getItem(
            "phoenix-command-current-character"
        )
...
export function displayWeapons(character) {
    ...
    _.forEach(character.weapons, (weapon) => {
        let gun = _.find(databaseWeapons, (o) => {
            return o.Name === weapon
        })
        ...
        _.forEach(ammoTypes, (ammoType) => {
            ammoDiv += `<option value="${ammoType}">${ammoType}</option>`
        })
        ammoDiv += "</select>"
        ...
        _.forEach(gunKeys, (aim) => {
            let tr = `
              <tr>
                  ...
                  <td id="aim-time-mod-${aim}" class="text-center">${
                gun["Aim Time"][aim]
            }</td>
                  <td id="shot-accuracy-${aim}" class="text-center">${
                gun["Aim Time"][aim] + character.sal
            }</td>
              </tr>
          `
            aimTime += tr
        ...
        $(`#reload-${ammoDropdown}`).click((e) => {
            ...
            Utils.log(
                `${character.name} reloaded the ${name} to capacity at ${cap} round(s).`
            )
